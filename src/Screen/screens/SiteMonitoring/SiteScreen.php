...
final class SiteScreen extends Screen {
    ...
    protected function siteScreen(int $siteID) {
        ...
        $message = $this->text->concatEOL(
            ...
            $site->getRawState() == 0 && $site->getLastCheck() != 0 ? '❗️ ' . $this->text->sprintf('Перестал работать %s.', $site->getFirstAlertSentTime()): '',
            $site->getRawState() == 0 && $site->getLastCheck() != 0 ? '▶️ ' . $this->prepareReason($site->getDownStateReason()): '',
            ($site->getLastCheck() == 0 ? 'Сайт ожидает своей первой проверки. Данные будут позже.' : ''),
        );
