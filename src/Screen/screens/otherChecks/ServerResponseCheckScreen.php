...
final class ServerResponseCheckScreen extends Screen implements ScreenInterface {
    ...
    protected string $screenName = 'ServerResponseCheck';
        ...
        switch ($state) {
            case 0:
                ...
                return $this->maybeSideExecute(array(
                    'Этот инструмент позволяет проверить ответ сервера.',
                    '',
                    '',
                    '<b>Отправьте домен</b>',
                    ''
    ...
    private function proccessCheckRequest($conversation) {
        ...
        $imageMessage = Request::sendPhoto(array(
            'chat_id' => $this->getChatID(),
            'photo' => Path::toAssets() . '/imgs/progress.png'
        ));
        ...
        try {
            $tool   = new ServerResponseCheckTool($inputDomain);
        ...
        $report = new CurlReport($result);

        if ($reportImage = $report->getReportImage()) {
            $toMember = Request::editMessageMedia(array(
                'chat_id' => $this->getChatID(),
                'message_id' => $imageMessage->getResult()->getMessageId(),
                'media' => new InputMediaPhoto(array(
                    'media' => $reportImage,
                )),
            ));
        } else {
            $toMember = Request::editMessageMedia(array(
                ...
                'message_id' => $imageMessage->getResult()->getMessageId(),
                'media' => new InputMediaPhoto(array(
                    'media' => Path::toAssets() . '/imgs/no_screenshot.png',
                )),
            ));
        }

        $this->memberLastMessage($toMember);
