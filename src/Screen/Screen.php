...
abstract class Screen implements ScreenInterface {
    ...
    public function setConversation(Conversation $conversation) {
        if (isset($this->conversation)) throw new ScreenException('Conversation is already set.');
     ...
     */
    final public function maybeSideExecute(string|array $text, Keyboard $reply_markup, bool $useTranslate = true, array $additionalData = array()): ServerResponse {
        ...
        if ($this->isSideExecute()) {
            ...
            return Request::editMessageText(array_merge(array(
                'chat_id' => $chat_id,
        ...
        return Request::sendMessage(array_merge(array(
            'chat_id' => $chat_id,
        ), $mergedData));
    }
     ...
     * @param array $additionalData         Optional. Additional data that will be sent with the message to Telegram.
     */
    final protected function tryToSendMessage(string $text, InlineKeyboard|Keyboard|false $reply_markup = false, array $additionalData = array()): ServerResponse {
        ...
        if ($reply_markup !== false) array_merge($passedData, array('reply_markup' => $reply_markup));
    ...
    final protected function removeLastMessage($minusIncrement = 0): ServerResponse {
        ...
        if ($message_id == null) return Request::emptyResponse();
        $message_id -= $minusIncrement;
        
        return Request::deleteMessage(array(
            'chat_id' => $chat_id,
            'message_id' => $message_id,
        ));
    }
    ...
    final protected function getChatID(): int {
        if ($this->isSideExecute()) return $this->command->getCallbackQuery()->getFrom()->getId();
    ...
    final protected function tryToSendTempMessage(string $text, int $timeout = 10): bool {
        ...
        if (!Environment::var('screen_settings')['useTempMessages']) return false;
