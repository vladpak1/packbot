...
class Curl {
    ...
    public function execute(): self {
        if ($this->isExecuted) throw new CurlException('Cannot execute curl request: curl request is already executed.');
        $this->realExecute();
        return $this;
    }
    ...
    public function getResponse(): CurlResponse {
        if (!$this->isExecuted) throw new CurlException('Cannot get response: curl request is not executed.');
    ...
    public function isOK(): bool {
        return $this->getResponse()->getCode() === 200;
    }
    ...
    protected function realExecute() {
        ...
        if (!$response || !$curlInfo) $this->curlError = curl_error($ch);
