...
class SiteManager
{
    ...
    public function addSite(string $domain): bool
    {
         ...
         */
        if (Url::hasPort($effectiveUrl)) {
            throw new SiteMonitoringException($this->text->sprintf('Эффективный адрес (%s) сайта содержит порт, что недопустимо для этого бота. Попробуйте еще раз.', $effectiveUrl));
        }

        /**
         * Check if user has exceeded the limit of sites.
         */
        if (SiteMonitoringDB::getUsersSitesCount($this->userID) >= Environment::var('monitoring_settings')['maxSitesPerUser']) {
            throw new SiteMonitoringException($this->text->sprintf('Вы можете добавить не более %d сайтов в мониторинг.', Environment::var('monitoring_settings')['maxSitesPerUser']));
        }

        if (SiteMonitoringDB::isSiteAssignedToUser($effectiveUrl, $this->userID)) {
            throw new SiteMonitoringException($this->text->e('Сайт уже добавлен в мониторинг.'));
        }
