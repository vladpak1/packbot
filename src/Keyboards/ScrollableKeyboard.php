...
class ScrollableKeyboard extends InlineKeyboard {
     ...
     */
    protected array $additionalButtons = array();
     ...
     * 
     * @param array $additionalButtons Additional buttons to be shown at the bottom of the keyboard.
     */
    ...
    public function getKeyboard(): InlineKeyboard {
        if (count($this->entries) == 0) throw new KeyboardException('No entries were added to the keyboard.');
    ...
    protected function constructKeyboard(): InlineKeyboard {
        ...
        return new InlineKeyboard(...array_merge($entriesButtons, array($navigationButtons), array($this->additionalButtons)));
    }
    ...
    protected function entriesToButtons(array $entries): array {
        ...
        foreach ($entries as $entry) {
            if (!isset($entry['text']) || !isset($entry['id'])) throw new KeyboardException('Invalid entry structure.');
            $buttons[] = array(
                array(
                    'text' => $entry['text'],
