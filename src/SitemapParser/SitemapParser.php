...
class SitemapParser
{
    ...
    {
        if ($this->isExecuted) {
            throw new SitemapParserException('SitemapParser is already executed.');
        }
    ...
    protected function getSitemapXmlFileAddress(string $url): array
    {
        try {
            ...
            if (!$response->isOK()) {
                throw new SitemapParserException('Failed to get robots.txt file.');
            }

            $robotsTxt = $response->getBody();

            if (0 == strlen($robotsTxt)) {
                throw new SitemapParserException('Robots.txt file is empty.');
            }
            ...
            if (0 == count($sitemaps)) {
                throw new SitemapParserException('Robots.txt file does not contain sitemap.xml file address.');
            }
    ...
    {
        if (false === $this->telegramMessageData) {
            return;
        }
    ...
    protected function getSitemapContent(string $sitemap)
    {
        try {
            ...
            if (!$response->isOK()) {
                throw new SitemapParserException($this->text->sprintf('Не удалось получить контент сайтмапа %s, код ответа %s', $sitemap, $response->getCode()));
            }
    ...
    protected function iterateSitemap(string $sitemapLink)
    {
        ...
        if (false === $xml) {
            throw new SitemapParserException($this->text->sprintf('Не удалось обработать сайтмап %s.', $sitemapLink));
        }

        if ($this->sitemapsIterated >= $this->maxSitemapsAtOnce) {
            throw new SitemapParserException($this->text->sprintf('Достигнуто максимальное количество сайтмапов (%d).', $this->maxSitemapsAtOnce));
        }
